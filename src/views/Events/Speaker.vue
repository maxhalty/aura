  ...
  <v-content>
    <v-container fluid class="py-0 my-0 mt-0">
      ...
      <v-row justify="center" align="center" class="py-5" v-else-if="sData.length<=0 && !isLoading">
        <v-col cols="12" md="12">
          <h2 :class="this.$vuetify.theme.dark?'white--text':'grey--text text--darken-3'" class="google-font mb-0">Ninguna tarjeta disponible.</h2>
      ...
      <v-row class="py-0 my-0" v-else>
        <v-col md="12" sm="12" cols="12" class="py-0 my-0">
          <v-row class="mt-5">
            <v-col class="">
              <h2 :class="this.$vuetify.theme.dark?'white--text':'grey--text text--darken-3'" class="google-font mb-0">Mira nuestras opciones</h2>
            </v-col>
          </v-row>
